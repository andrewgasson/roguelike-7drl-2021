...
void InitDoors(int capacity)
{
	...
	doorStatus = MemAlloc(capacity * sizeof(*doorStatus));
	doorData = MemAlloc(capacity * sizeof(*doorData));
...
inline int MaxDoors(void)
{
	return doorCapacity;
}

inline bool IsDoorValid(Handle door)
{
...
inline TerminalTile GetDoorClosedTile(Handle door)
{
	return doorData[door.index].closedTile;
}

inline TerminalTile GetDoorOpenedTile(Handle door)
{
	return doorData[door.index].openedTile;
}

inline Vector2 GetDoorPosition(Handle door)
{
...
inline bool IsDoorOpened(Handle door)
{
	return doorData[door.index].opened;
}

inline void SetDoorOpenedTile(Handle door, TerminalTile openedTile)
{
...
inline void SetDoorOpened(Handle door, bool opened)
{
...
inline void SetDoorPosition(Handle door, Vector2 position)
{
...
static void CacheActiveDoors(int *outLength, Handle *outHandles[])
{
	...
	*outLength = 0;
	*outHandles = MemAlloc(doorCount * sizeof(**outHandles));
